...
export const darkCratersScreenAudit: readonly DarkCratersScreenAuditEntry[] = [
  {
    ...
    intendedComposition: [
      ...
      "Image-first composition when the plate already includes DARK CRATERS title art and quarantine signage.",
      "App UI adds Play/action affordances only; the baked title, moon, outpost, and quarantine sign stay visible.",
      "Sparse access/status copy may sit near Play but must not become a second title, tagline, or warning card.",
    ],
    forbiddenRegression: "Second DARK CRATERS title over baked title, separate THE MOON REMEMBERS tagline over baked art, second warning card over baked warning sign, heavy overlay burying the background, large opaque menu card fighting the composition, hidden or weak Play button, or changed Play/audio initialization behavior.",
    validationChecklist: [
      ...
      "Only the baked background title is visible when the branded start-screen background is active.",
      "Only the baked quarantine sign is visible when the branded start-screen background is active.",
      "Play is the obvious primary action and remains wired to the existing audio/init flow.",
      "Secondary/status copy is sparse and does not cover the moon, outpost, title, or signage focal detail.",
  ...
  },
  {
    ...
    intendedComposition: [
      "Launch authorization terminal bridging Habitat Review Assignment to Begin Descent.",
      "Top assignment chrome summarizes run tier, family, route, ship, and gear readiness.",
      "Left rail presents one concise TYCHOSTAR field order, center stages the selected runner inside a deployment bay, and right rail answers descent readiness.",
      "Bottom command strip keeps Return to Habitat, Change Operation, Review Loadout, Ship Prep, and Begin Descent visible, with Begin Descent strongest.",
      "Future descent language points toward a third-person Kestrel-9 ship cinematic over fake lunar approach video, not cockpit POV.",
    ],
    forbiddenRegression: "Generic ready-check database layout, class selection cards reappearing, hidden Begin Descent, clipped side rails, mission data repeated into text walls, model floating on a flat grid, or direct raid bypass from Habitat Hub.",
    validationChecklist: [
      "02-deployment-assignment.png reads as launch authorization before reading labels.",
      "02b-deployment-assignment-1366.png, if captured, keeps Begin Descent, rails, and model stage visible.",
      "Mission title/family/route/gear readiness match the Habitat Hub and Crater Runs context.",
      "Class model remains mounted through the existing preview lifecycle and is staged in a deployment bay.",
      "Begin Descent remains the final action and uses the existing deployment action path.",
      "Return to Habitat, Change Operation, Review Loadout, and Ship Prep remain visible.",
    ],
  },
  {
    screen: "descentSequence",
    label: "Descent Sequence",
    priority: 1,
    expectedArchetype: "descent-bridge",
    reference: "public/design-reference/ui/reference-game-sequence.png panel 5 / DESCENT + LANDING",
    intendedComposition: [
      "Drift-Wave-like third-person exterior Kestrel-9 chase bridge between Deployment Assignment and active raid gameplay.",
      "Flying Kestrel-9 at /models/ships/kestrel-9-inflight.glb is the visible ship asset; landed or gear-down ship is not used while that asset exists.",
      "Camera trails behind and above with a rear three-quarter hull read, forward lead, damping, and subtle ship bank rather than a static side display.",
      "Imported ship nodes, runtime material, and light remain under descent-only ownership and cannot mutate world/map/proxy materials or visibility.",
      "Fake lunar approach layer sits behind/around the ship to sell orbital-to-surface scale without rendering the full lunar map from orbit.",
      "Ship, HUD, telemetry, landing quality feedback, and final handoff remain game-rendered and game-controlled.",
      "Future video/image layer follows accepted GLB isolation and chase framing; it may be MP4, WebM, poster fallback, CSS fallback, or image sequence, but must not hard-crash when missing.",
    ],
    forbiddenRegression: "Map/proxy structures disappearing after ship import, scene-wide material/light sweeps, direct structure GLBs re-enabled, procedural ground-only view, generic box ship while the Kestrel GLB exists, cockpit POV, first-person-only landing footage, side-on static model, head-on blob, hidden-ship moon-only footage, camera-under-lander framing, hard-required missing video assets, changed Begin Descent action, blocked raid entry, changed raid launch timing, or bypassed landing quality authority.",
    validationChecklist: [
      "Begin Descent enters the existing deployment flow without changing selected mission or raid start behavior.",
      "Kestrel-9 remains visible in third-person exterior framing during the intended descent presentation.",
      "12-descent-bridge.png shows the inflight Kestrel-9 in recognizable third-person exterior framing.",
      "12b-descent-active-raid-after-skip.png shows active raid HUD and visible world geometry after ship teardown/hide.",
      "Fake approach layer is optional/fallback-safe until real video or image assets exist.",
      "HUD telemetry remains readable and represents descent progress, alignment, landing quality, and skip/debug affordance.",
      "Current K skip/debug landing controls remain preserved unless a later accessibility/debug phase explicitly changes them.",
    ],
  },
  {
