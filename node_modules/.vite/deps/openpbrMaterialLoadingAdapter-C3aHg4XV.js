...
import { a as ExtractChannelAsync, c as LerpTexturesAsync, d as TextureChannel, f as TextureColorSpace, i as CreateTextureWithFactorOperand, o as ExtractMaxChannelAsync, s as InvertTextureAsync, t as ChannelMask, u as MultiplyTexturesAsync } from "./textureProcessor-DWcKRqgd.js";
//#region node_modules/@babylonjs/loaders/glTF/2.0/openpbrMaterialLoadingAdapter.js
var openpbrMaterialLoadingAdapter_exports = /* @__PURE__ */ __exportAll({ OpenPBRMaterialLoadingAdapter: () => OpenPBRMaterialLoadingAdapter });
/**
...
var OpenPBRMaterialLoadingAdapter = class {
	...
	*/
	set specularWeightTexture(value) {
		...
		} else this._material.specularWeightTexture = value;
	}
	/**
	...
	*/
	set coatRoughnessTexture(value) {
		...
		if (value) this._material._useCoatRoughnessFromGreenChannel = true;
	}
	/**
	...
	*/
	set geometryCoatTangentTexture(value) {
		...
		if (value) this._material._useCoatRoughnessAnisotropyFromTangentTexture = true;
	}
	/**
	...
	*/
	set transmissionDepth(value) {
		if (value !== Number.MAX_VALUE || this._material.transmissionDepth !== 0) this._material.transmissionDepth = value;
		else this._material.transmissionDepth = 0;
	}
	/**
	...
	*/
	set transmissionColor(value) {
		if (!value.equals(Color3.White())) this._material.transmissionColor = value;
	}
	/**
	...
	*/
	set geometryCoatNormalTextureScale(value) {
		if (this._material.geometryCoatNormalTexture) this._material.geometryCoatNormalTexture.level = value;
	}
	/**
	...
	*/
	async finalizeAsync(signal) {
		if (this._diffuseTransmissionTint && !this._diffuseTransmissionTint.equals(Color3.White()) || this._diffuseTransmissionTintTexture) if (this._material.geometryThinWalled) {
		...
		if (this._specWorkflow) {
			...
			this.specularColorTexture?.dispose();
	...
	async copySurfaceToCoatAsync(weight, weightTexture, weightTextureChannel, color, colorTexture, diffuseTransmission = false, signal = new AbortController().signal) {
		...
		if (newCoatColor.texture) {
			...
			this.coatColor.fromArray([
				newCoatColor.factor.r,
				newCoatColor.factor.g,
				newCoatColor.factor.b
			]);
		}
		...
		if (origCoatNormalTexture || this.geometryNormalTexture) {
			const newCoatNormal = await LerpTexturesAsync("newCoatNormal (" + this._material.name + ")", CreateTextureWithFactorOperand(this.geometryNormalTexture, this.geometryNormalTexture ? new Color4(1, 1, 1, 1) : new Color4(.5, .5, 1, 1), TextureChannel.RGBA), CreateTextureWithFactorOperand(origCoatNormalTexture, origCoatNormalTexture ? new Color4(1, 1, 1, 1) : new Color4(.5, .5, 1, 1), TextureChannel.RGBA), CreateTextureWithFactorOperand(origCoatWeightTexture, origCoatWeightCol4, TextureChannel.R), this._material.getScene());
			if (signal.aborted) {
				newCoatNormal.texture?.dispose();
				return;
			}
			if (newCoatNormal.texture) this.geometryCoatNormalTexture = newCoatNormal.texture;
		}
	}
};
//#endregion
export { openpbrMaterialLoadingAdapter_exports as n, OpenPBRMaterialLoadingAdapter as t };

//# sourceMappingURL=openpbrMaterialLoadingAdapter-C3aHg4XV.js.map