...
import { c as InternalTexture } from "./tools.functions-BjbqYpqF.js";
import { t as ThinEngine } from "./thinEngine-CjVUVlFh.js";
//#region node_modules/@babylonjs/core/Engines/Extensions/engine.multiRender.js
...
ThinEngine.prototype.buildTextureLayout = function(textureStatus, backBufferLayout = false) {
	const gl = this._gl;
	const result = [];
	if (backBufferLayout) result.push(gl.BACK);
	else for (let i = 0; i < textureStatus.length; i++) if (textureStatus[i]) result.push(gl["COLOR_ATTACHMENT" + i]);
	else result.push(gl.NONE);
	return result;
};
...
ThinEngine.prototype.unBindMultiColorAttachmentFramebuffer = function(rtWrapper, disableGenerateMipMaps = false, onBeforeUnbind) {
	...
	if (!rtWrapper.disableAutomaticMSAAResolve) this.resolveMultiFramebuffer(rtWrapper);
	if (!disableGenerateMipMaps) this.generateMipMapsMultiFramebuffer(rtWrapper);
	if (onBeforeUnbind) {
		if (rtWrapper._MSAAFramebuffer) this._bindUnboundFramebuffer(rtWrapper._framebuffer);
		onBeforeUnbind();
	}
...
ThinEngine.prototype.createMultipleRenderTarget = function(size, options, initializeBuffers = true) {
	...
	if (options !== void 0) {
		...
		if (this.webGLVersion > 1 && (options.depthTextureFormat === 13 || options.depthTextureFormat === 17 || options.depthTextureFormat === 16 || options.depthTextureFormat === 14 || options.depthTextureFormat === 18)) depthTextureFormat = options.depthTextureFormat;
	}
	if (depthTextureFormat === void 0) depthTextureFormat = generateStencilBuffer ? 13 : 14;
	const gl = this._gl;
	...
	for (let i = 0; i < textureCount; i++) {
		...
		if (isWebGL2 && (target === 35866 || target === 32879)) {
			if (target === 35866) texture.is2DArray = true;
			else texture.is3D = true;
			...
			for (let i = 0; i < 6; i++) gl.texImage2D(gl.TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, internalSizedFormat, width, height, 0, internalFormat, webGLTextureType, null);
			texture.isCube = true;
		} else gl.texImage2D(gl.TEXTURE_2D, 0, internalSizedFormat, width, height, 0, internalFormat, webGLTextureType, null);
		if (generateMipMaps) gl.generateMipmap(target);
	...
	if (generateDepthTexture && this._caps.depthTextureExtension && !dontCreateTextures) {
		const depthTexture = new InternalTexture(this, 14);
		...
		if (this.webGLVersion < 2) glDepthTextureInternalFormat = gl.DEPTH_COMPONENT;
		else if (depthTextureFormat === 14) {
			depthTextureType = 1;
	...
	if (initializeBuffers) gl.drawBuffers(attachments);
	this._bindUnboundFramebuffer(currentFramebuffer);
	rtWrapper.setLayerAndFaceIndices(layerIndex, faceIndex);
	this.resetTextureCache();
	if (!dontCreateTextures) this.updateMultipleRenderTargetTextureSampleCount(rtWrapper, samples, initializeBuffers);
	else if (samples > 1) {
		const framebuffer = gl.createFramebuffer();
...
ThinEngine.prototype.updateMultipleRenderTargetTextureSampleCount = function(rtWrapper, samples, initializeBuffers = true) {
	...
	for (let i = 0; i < count; i++) rtWrapper.textures[i]._hardwareTexture?.releaseMSAARenderBuffers();
	if (samples > 1 && typeof gl.renderbufferStorageMultisample === "function") {
		...
		if (initializeBuffers) gl.drawBuffers(attachments);
	} else this._bindUnboundFramebuffer(rtWrapper._framebuffer);
...
ThinEngine.prototype.resolveMultiFramebuffer = function(texture) {
	...
	for (let i = 0; i < count; i++) {
		const texture = rtWrapper.textures[i];
		for (let j = 0; j < count; j++) attachments[j] = gl.NONE;
	...
	for (let i = 0; i < count; i++) attachments[i] = gl[this.webGLVersion > 1 ? "COLOR_ATTACHMENT" + i : "COLOR_ATTACHMENT" + i + "_WEBGL"];
	gl.drawBuffers(attachments);
	gl.bindFramebuffer(this._gl.FRAMEBUFFER, rtWrapper._MSAAFramebuffer);
};
//#endregion
export { engine_multiRender_exports as t };

//# sourceMappingURL=engine.multiRender--5pYDmuz.js.map